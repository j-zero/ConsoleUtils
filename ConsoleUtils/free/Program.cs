...
namespace free
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            float usedRam = installedRam - availableRam;
            Console.WriteLine($"CPU Usage:\t{cpuCounter.NextValue()}%");
            Console.WriteLine($"RAM Usage:\t{usedRam}/{installedRam}MB, {availableRam}MB free");
            ...
            foreach (DriveInfo drive in drives)
            {
                if (drive.IsReady) {
                    ...
                    string driveReadableFree = CalculateHumanReadableSize(drive.TotalFreeSpace);
                    string driveReadableTotalSize = CalculateHumanReadableSize(drive.TotalSize);
                    

                    Console.WriteLine($"\t{drive.Name} ({drive.DriveType}):\t {driveReadableFree}/{driveReadableTotalSize}\t({String.Format("{0:0.00}", percentFree)}%) free");
        ...
        private static string CalculateHumanReadableSize(Int64 value, int factor = 1024, int decimalPlaces = 1)
        {
        ...
        private static readonly string[] SizeSuffixes = { "", "k", "M", "G", "T", "P", "E", "Z", "Y" };
